...
<body>
	...
	<div>
			...
			<tbody>
				<c:choose>
					<c:when test="${ empty  list }">
						<tr>
							<td colspan="5" align="center">no board...
							</td>
						</tr>
					</c:when>
					<c:otherwise>
			...
			<tfoot>
				<tr>
					...
					<form id="searchForm" action="/board/list" method="GET">
									<select id="type" name="type" id="selType">
										...
										<option value="TC" ${ pageMaker.criteria.type == 'TC' ? 'selected' : '' }>제목 or내용</option>							
										<option value="TW" ${ pageMaker.criteria.type == 'TW' ? 'selected' : '' }>제목 or 작성자</option>							
										<option value="TWC" ${  pageMaker.criteria.type == 'TWC' ? 'selected' : '' }>제목 or 작성자 or 내용</option>							
									</select>
									<input type="text" name="keyword" style="padding: 7px" value="${ pageMaker.criteria.keyword }">
		...
		<form id="actionForm" action="/board/list" method="GET">
			...
			<input type="hidden" name="type" value="${ pageMaker.criteria.type }" style="padding: 7px">
			<input type='hidden' name='pageNum' value="${ pageMaker.criteria.pageNum }">
			<input type='hidden' name='amount' value="${ pageMaker.criteria.amount }">
		</form>
	</div>
